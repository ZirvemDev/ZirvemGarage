<template>
    <div class="bg-gray-100 flex" v-if="authStore.user && authStore.user.hasOwnProperty('id')">
        <aside class="relative h-screen w-64 hidden sm:block shadow-xl" style="background-color:#35495e;">
            <div class="p-6 border-b border-theme-500">
            ...
            <template v-if="state.footerLeftLink">
                <a v-if="state.footerLeftLink.href" :href="state.footerLeftLink.href" class="absolute w-full bottom-0 bg-theme-500 text-white flex items-center justify-center py-4">
        ...
        <div class="relative w-full flex flex-col h-screen overflow-y-hidden">
            ...
            <header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
                ...
                <div class="relative w-1/2 flex justify-end">
                    ...
                    <div v-if="state.isAccountDropdownOpen" class="absolute w-32 bg-white rounded-lg shadow-lg py-2 mt-16 z-50">
                        <router-link to="/panel/profile" class="block px-4 py-2 hover:bg-theme-500 hover:text-white hover:opacity-80">
                            {{ trans('global.pages.profile') }}
                        </router-link>
                        <a href="#" @click.prevent="onLogout" class="block px-4 py-2 hover:bg-theme-500 hover:text-white hover:opacity-80">{{
            ...
            <header class="w-full bg-theme-600 py-5 px-6 sm:hidden">
                ...
                <nav :class="state.isMobileMenuOpen ? 'flex': 'hidden'" class="flex flex-col pt-4 text-base text-white">
                    ...
                    <button class="w-full bg-theme-500 text-white font-semibold py-2 mt-3 rounded-lg shadow-lg hover:shadow-xl hover:text-theme-800 hover:bg-gray-300 flex items-center justify-center">
        ...
        const state = reactive({
            mainMenu: [
                ...
                },
                {
                    ...
                    showDesktop: true,
                    showMobile: true,
                    requiresAbility: getAbilitiesForRoute(['roles.view', 'roles.create', 'roles.edit']),
                    to: '/panel/roles',
                    children: [
                        {
                            name: trans('roles.roles_list'),
                            icon: '',
                            showDesktop: true,
                            showMobile: true,
                            requiresAbility: getAbilitiesForRoute('roles.view'),
                            to: '/panel/roles',
                        },
                        {
                            name: trans('roles.create_role'),
                            icon: '',
                            showDesktop: true,
                            showMobile: true,
